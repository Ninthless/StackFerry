...
Only download StackFerry from **[GitHub Releases](https://github.com/Ninthless/StackFerry/releases)** or the project source repository. Any "StackFerry" site or client that asks for payment, top-ups, or login credentials is not official.
...
The CLI tools managed by StackFerry (Claude Code, Codex, Gemini CLI) require a Node.js environment.
...
1. Visit the [Releases page](https://github.com/Ninthless/StackFerry/releases)
...
2. Open the DMG, or extract the zip to get `StackFerry.app`
3. Drag it to the Applications folder

### Signed and Notarized

StackFerry for macOS is signed and notarized by Apple. You can install and open it directly — no extra steps needed.
...
After installation, launch StackFerry:

1. The app window displays correctly
2. A StackFerry icon appears in the system tray
3. The app switcher shows enabled managed apps, and you can switch to the target app panel

## Auto Update

StackFerry includes built-in auto-update functionality:
...
- Move `StackFerry.app` to Trash
- Optional: Delete the configuration directory `~/.stackferry/`
