...
App routing means letting StackFerry route a specific application's API requests through the local routing service.
...
When routing is enabled, StackFerry modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, StackFerry will:
...
Before enabling routing, StackFerry backs up the original configuration and restores it when disabled.
