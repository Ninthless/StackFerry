...
## StackFerry Data Storage

### Storage Directory

Default location: `~/.stackferry/`
...
```
~/.stackferry/
├── stackferry.db      # SQLite database (SSOT)
...
`stackferry.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. StackFerry writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to StackFerry's "providers") |
...
StackFerry's priority when modifying configurations:

1. **StackFerry Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by StackFerry)
- StackFerry's `settings.json`

### Not Recommended to Edit Manually

- `stackferry.db` database file
- Backup files
...
StackFerry v3.7.0 migrated from JSON files to SQLite:
