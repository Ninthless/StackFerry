...
StackFerry for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/Ninthless/StackFerry/releases).
...
2. Add StackFerry to your antivirus software's whitelist
...
**Solutions**:
1. Check if the `~/.stackferry/` directory exists
2. Restore from backup: `~/.stackferry/backups/`
...
**Solutions**:
1. Confirm the file was exported by StackFerry
...
**Windows**:
- Check taskbar settings to ensure the StackFerry icon is not hidden
...
2. Restart the app
3. Delete `~/.stackferry/settings.json` to reset settings
...
Toggle "Lightweight Mode" from the system tray menu. The main window closes, and StackFerry runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
1. Visit [GitHub Issues](https://github.com/Ninthless/StackFerry/issues)
...
StackFerry stores logs under its application configuration directory. By default this is `.stackferry` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.

When submitting an Issue, attach the files relevant to the problem:

- General, network, or proxy errors: `~/.stackferry/logs/stackferry.log` and its rotated files
- Application crashes: `~/.stackferry/crash.log`, `crash.log.1`, and `crash.log.2`
- The default Windows location is `C:\Users\<username>\.stackferry\...`
