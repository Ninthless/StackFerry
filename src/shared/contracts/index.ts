...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.stackferry/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
import type { TemplateType } from "@/shared/contracts/provider";
export {
  isOAuthProviderType,
...
// Skill 存储位置
export type SkillStorageLocation = "stack_ferry" | "unified";
...
// 存储在本地 ~/.stackferry/settings.json，不随数据库同步
export interface Settings {
  ...
  // Skill 存储位置：stack_ferry（默认）或 unified（~/.agents/skills/）
