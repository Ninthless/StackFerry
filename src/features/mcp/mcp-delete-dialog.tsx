...
export function McpDeleteDialog({ name, onOpenChange, onConfirm }: Props) {
    ...
    <AlertDialog open={Boolean(name)} onOpenChange={(open) => !open && onOpenChange(false)}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>{m.mcp_delete_description({ name: displayed })}</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
