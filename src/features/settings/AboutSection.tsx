...
import { runtimeApi, settingsApi } from "@/platform/tauri/api";
import type {
  ToolInstallation,
  ToolInstallationReport,
} from "@/platform/tauri/api/settings";
...
export function AboutSection({ isPortable }: AboutSectionProps) {
  ...
  useEffect(() => {
    ...
    const loadAppVersion = async () => {
      try {
        const appVersion = await runtimeApi.getVersion();
    ...
    try {
      ...
      if (!displayVersion) {
        await settingsApi.openExternal(
          "https://github.com/Ninthless/StackFerry/releases",
        );
        return;
      }

      await settingsApi.openExternal(
        `https://github.com/Ninthless/StackFerry/releases/tag/${displayVersion}`,
      );
    } catch (error) {
          ...
          <div className="flex items-center gap-8">
            <div className="flex flex-col items-center gap-2">
              <div className="flex items-center gap-2">
                <img src={appIcon} alt="StackFerry" className="h-5 w-5" />
            ...
            <Button
              ...
              onClick={() =>
                settingsApi.openExternal(
                  "https://github.com/Ninthless/StackFerry",
                )
              }
