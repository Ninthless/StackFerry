...
import { runtimeApi, settingsApi, type AppId } from "@/platform/tauri/api";
...
const computeDefaultAppConfigDir = async (): Promise<string | undefined> => {
  try {
    const home = await runtimeApi.homeDir();
    return await runtimeApi.joinPath(home, ".stackferry");
  } catch (error) {
    console.error(
...
const computeDefaultConfigDir = async (
  ...
  try {
    const home = await runtimeApi.homeDir();
    return await runtimeApi.joinPath(
      home,
      APP_DIRECTORY_META[app].defaultFolder,
    );
  } catch (error) {
    console.error(
