...
import type { LegacyImportCandidate } from "@shared/types"
...
export function LegacyImportCard() {
  const [detected, setDetected] = useState<LegacyImportCandidate | null>(null)
  ...
  const detect = useCallback(async () => {
    ...
    if (!api) return
    try {
      setDetected(await api.detectLegacyImport())
    } catch {
      setDetected(null)
    } finally {
      setLoading(false)
  ...
  async function choose(): Promise<void> {
    ...
    if (!api) return
    try {
      const next = await api.chooseLegacyImport()
  ...
  async function runImport(): Promise<void> {
    ...
    setImporting(true)
    try {
      const result = await api.importLegacyProviders(candidate.dbPath)
      setCandidate(null)
      toast.add({
        type: "success",
        description: m.toast_legacy_imported({
  ...
  return (
    <Card>
      <CardHeader>
        <HintTitle hint={m.legacy_description()}>
          <CardTitle>{m.legacy_legend()}</CardTitle>
        </HintTitle>
        <CardAction>
      ...
      <CardContent>
        {loading ? null : detected ? (
          ...
          <Alert>
            <AlertDescription>{m.legacy_none()}</AlertDescription>
          </Alert>
        )}
      </CardContent>
      <AlertDialog open={candidate !== null} onOpenChange={(open) => (!open ? setCandidate(null) : undefined)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogTitle>{m.legacy_confirm_title()}</AlertDialogTitle>
            <AlertDialogDescription>
              {m.legacy_confirm_description({
