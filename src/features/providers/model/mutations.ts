...
import type { DeleteSessionOptions } from "@/platform/tauri/api/sessions";
import type { SwitchResult } from "@/platform/tauri/api/providers";
import type { Provider, SessionMeta, Settings } from "@/shared/contracts";
...
export const useAddProviderMutation = (appId: AppId) => {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      ...
      if (appId === "openclaw") {
        await queryClient.invalidateQueries({
          queryKey: openclawProviderKeys.health,
        });
      }
...
export const useSwitchProviderMutation = (appId: AppId) => {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      ...
      if (appId === "openclaw") {
        await queryClient.invalidateQueries({
          queryKey: openclawProviderKeys.liveProviderIds,
        });
        await queryClient.invalidateQueries({
          queryKey: openclawProviderKeys.defaultModel,
        });
        await queryClient.invalidateQueries({
          queryKey: openclawProviderKeys.health,
        });
