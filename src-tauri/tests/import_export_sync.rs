...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    let auth_path = stackferry_lib::get_codex_auth_path();
    let config_path = stackferry_lib::get_codex_config_path();
...
requires_openai_auth = true
"#;
    stackferry_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(stackferry_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    stackferry_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = stackferry_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        stackferry_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        stackferry_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        stackferry_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !stackferry_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !stackferry_lib::get_codex_config_path().exists(),
...
"#;
    ...
    stackferry_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        stackferry_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = stackferry_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        stackferry_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        stackferry_lib::AppError::IoContext { context, .. } => {
            assert!(
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    stackferry_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = stackferry_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = stackferry_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".stackferry").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".stackferry");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.stackferry/settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("stackferry-test-invalid<>dir")
    } else {
