...
pub(crate) fn reset_codex_usage_on_conn(
    ...
    if table_exists(conn, "proxy_request_logs")?
        && column_exists(conn, "proxy_request_logs", "data_source")?
    {
        conn.execute(
    ...
    if table_exists(conn, "usage_daily_rollups")?
        && column_exists(conn, "usage_daily_rollups", "provider_id")?
    {
        conn.execute(
    ...
    if table_exists(conn, "session_log_sync")?
        && column_exists(conn, "session_log_sync", "file_path")?
    {
        let paths = {
