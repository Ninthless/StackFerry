...
impl Database {
    pub fn save_agent_instance(&self, instance: &AgentInstance) -> Result<(), AppError> {
        ...
        conn.execute(
            "INSERT INTO agent_instances (
                id, provider_id, app_type, name, credential_ref, codex_home, created_at, updated_at
    ...
    pub fn get_agent_instance(&self, id: &str) -> Result<Option<AgentInstance>, AppError> {
        let conn = lock_conn!(self.conn);
        conn.query_row(
            "SELECT id, provider_id, app_type, name, credential_ref, codex_home, created_at, updated_at
