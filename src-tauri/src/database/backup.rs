...
///
/// 头部校验（`validate_stackferry_sql_export`）只比较一个注释前缀，任何人都能在
...
impl Database {
    ...
    fn validate_stackferry_sql_export(sql: &str) -> Result<(), AppError> {
        let trimmed = sql.trim_start();
        if trimmed.starts_with(STACKFERRY_SQL_EXPORT_HEADER)
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 StackFerry 或 CC Switch 导出的 SQL 备份文件。",
            "Only SQL backups exported by StackFerry or CC Switch are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("stackferry.db");
        if !db_path.exists() {
            return Ok(None);
        }
...
mod tests {
    ...
    fn import_rejects_cross_file_statements_and_leaves_no_file_behind() -> Result<(), AppError> {
        ...
        for (label, template) in cases {
            let target = std::env::temp_dir().join(format!("stackferry-authorizer-{label}.sqlite"));
            ...
            let malicious = format!(
                "{}\n{}\n",
                super::STACKFERRY_SQL_EXPORT_HEADER,
    ...
    fn periodic_maintenance_runs_even_when_auto_backup_disabled() -> Result<(), AppError> {
        ...
        let test_home =
            std::env::temp_dir().join("stackferry-periodic-maintenance-backup-disabled-test");
        let _ = std::fs::remove_dir_all(&test_home);
        std::fs::create_dir_all(&test_home).expect("create test home");
        std::env::set_var("STACKFERRY_TEST_HOME", &test_home);
