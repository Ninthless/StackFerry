...
pub const STACKFERRY_CODEX_MODEL_PROVIDER_ID: &str = "custom";
...
/// this per the vendors' official Codex docs. Also doubles as StackFerry's
...
///
/// - `ProxyChat`: StackFerry's proxy takes over and converts Responses<->Chat,
...
pub enum CodexCatalogToolProfile {
    ProxyChat,
    NativeResponses,
    /// Codex talks (through StackFerry's proxy) to a native Anthropic Messages
...
pub fn get_codex_model_catalog_path() -> PathBuf {
    get_codex_config_dir().join(STACKFERRY_CODEX_MODEL_CATALOG_FILENAME)
}
...
fn set_codex_model_catalog_json_field(
    ...
    match catalog_path {
        Some(_) => {
            doc["model_catalog_json"] = toml_edit::value(STACKFERRY_CODEX_MODEL_CATALOG_FILENAME);
        }
        None => {
            let should_remove = doc
                ...
                .map(|path| {
                    Path::new(path).file_name().and_then(|name| name.to_str())
                        == Some(STACKFERRY_CODEX_MODEL_CATALOG_FILENAME)
                })
                .unwrap_or(false);
            if should_remove {
...
/// but only when it carries StackFerry's own `"disabled"` sentinel, so switching
...
/// lifecycle is bound to the StackFerry catalog pointer so the field is set/cleaned
...
/// StackFerry-maintained catalog file referenced by `~/.codex/config.toml` and
...
/// StackFerry-generated file (identified by filename
/// `stackferry-model-catalog.json`). A user-managed external catalog file is
...
pub fn read_codex_model_catalog_simplified_from_live() -> Result<Option<Value>, AppError> {
    ...
    let Some(catalog_path) = resolve_stackferry_catalog_path(&config_text, &generated_path) else {
        return Ok(None);
    };
...
/// Given `config.toml` text, resolve the on-disk path of the StackFerry-owned
...
pub(crate) fn resolve_stackferry_catalog_path(
    config_text: &str,
    generated_path: &Path,
) -> Option<PathBuf> {
    ...
    let is_stackferry_owned = referenced_path.file_name().and_then(|name| name.to_str())
        == Some(STACKFERRY_CODEX_MODEL_CATALOG_FILENAME);
    if !is_stackferry_owned {
        return None;
    }
...
pub fn apply_codex_official_proxy_route(
    ...
    doc.as_table_mut().remove("experimental_bearer_token");
    doc["model_provider"] = toml_edit::value(STACKFERRY_CODEX_OFFICIAL_PROXY_PROVIDER_ID);
    ...
    providers.insert(
        STACKFERRY_CODEX_OFFICIAL_PROXY_PROVIDER_ID,
...
pub fn codex_config_has_official_proxy_route(config_text: &str) -> bool {
    if !config_text.contains(STACKFERRY_CODEX_OFFICIAL_PROXY_PROVIDER_ID) {
        return false;
    }
    config_text
        .parse::<DocumentMut>()
        ...
        .as_deref()
        == Some(STACKFERRY_CODEX_OFFICIAL_PROXY_PROVIDER_ID)
}
...
pub fn remove_codex_official_proxy_route(config_text: &str) -> Result<String, AppError> {
    ...
    if let Some(item) = doc.as_table_mut().remove("model_providers") {
        ...
        providers.remove(STACKFERRY_CODEX_OFFICIAL_PROXY_PROVIDER_ID);
        remove_codex_proxy_placeholders_from_providers(&mut providers);
...
pub fn inject_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    let existing_custom_conflicts = doc
        .get("model_providers")
        .and_then(|item| item.as_table())
        .and_then(|providers| providers.get(STACKFERRY_CODEX_MODEL_PROVIDER_ID))
        .and_then(|item| item.as_table())
    ...
    if let Some(providers) = doc["model_providers"].as_table_mut() {
        if !providers.contains_key(STACKFERRY_CODEX_MODEL_PROVIDER_ID) {
            providers.insert(
                STACKFERRY_CODEX_MODEL_PROVIDER_ID,
...
pub fn strip_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    if doc.get("model_provider").and_then(|item| item.as_str())
        != Some(STACKFERRY_CODEX_MODEL_PROVIDER_ID)
    {
        return Ok(config_text.to_string());
    }
    ...
    let providers_empty = doc["model_providers"]
        .as_table_mut()
        .map(|providers| {
            providers.remove(STACKFERRY_CODEX_MODEL_PROVIDER_ID);
            providers.is_empty()
        })
        .unwrap_or(false);
    if providers_empty {
...
mod tests {
    ...
    fn unified_session_bucket_injects_for_empty_official_config() {
        ...
        let custom = doc["model_providers"][STACKFERRY_CODEX_MODEL_PROVIDER_ID]
            .as_table()
...
"#;
        ...
        let provider = &doc["model_providers"][STACKFERRY_CODEX_OFFICIAL_PROXY_PROVIDER_ID];
        assert_eq!(
...
"#;
        ...
        assert!(projected_doc["model_providers"]
            .get(STACKFERRY_CODEX_OFFICIAL_PROXY_PROVIDER_ID)
            .is_some());
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"stackferry-model-catalog.json\"\n";
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/stackferry-model-catalog.json");
...
"#;
    ...
    fn native_web_search_field_removes_own_sentinel_when_not_disabled() {
        ...
        // removing StackFerry's own "disabled" sentinel.
...
"#;
        ...
        assert!(
            ...
            "StackFerry's disabled sentinel should be removed when not native"
        );
    }
    ...
    fn native_web_search_field_preserves_user_value() {
        ...
        // only StackFerry's "disabled" sentinel is owned/removable.
...
"#;
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        let generated = PathBuf::from("/tmp/.codex/stackferry-model-catalog.json");
        assert!(resolve_stackferry_catalog_path("", &generated).is_none());
        assert!(
            resolve_stackferry_catalog_path("model = \"gpt-5\"", &generated).is_none(),
    ...
    fn resolve_catalog_path_accepts_cc_switch_owned_file() {
        let generated = PathBuf::from("/tmp/.codex/stackferry-model-catalog.json");
        let config = r#"model_catalog_json = "/tmp/.codex/stackferry-model-catalog.json"
"#;
        let resolved = resolve_stackferry_catalog_path(config, &generated).expect("path resolves");
...
"#;
        assert!(
            resolve_stackferry_catalog_path(config, &generated).is_none(),
...
"#;
        ...
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\stackferry-model-catalog.json");
        ...
        assert_eq!(
            written_path, STACKFERRY_CODEX_MODEL_CATALOG_FILENAME,
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/stackferry-model-catalog.json");
    ...
    fn set_catalog_json_none_removes_cc_switch_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/stackferry-model-catalog.json"
"#;
...
"#;
        let generated_path = PathBuf::from("/home/user/.codex/stackferry-model-catalog.json");
        let result = resolve_stackferry_catalog_path(config_text, &generated_path);
        assert_eq!(
            result,
            Some(generated_path),
...
"#;
    ...
    fn set_catalog_json_none_removes_relative_path() {
        let input = r#"model_catalog_json = "stackferry-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
